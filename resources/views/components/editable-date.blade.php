...
<div class="hover-container w-full flex flex-row gap-2"  x-data="editableInput('{{ $value }}', '{{ $field }}', '{{ $link }}', $dispatch)">
    ...
    <div class="inline-block gap-2 flex-1 flex">
        <input x-model="value" type="date" x-ref="field" x-bind:disabled="!isEditing" class="p-1 flex-1 text-sm border-none border-bottom border-gray-400 focus:border-gray-400 focus:ring-0" @keydown.enter="save()" @keydown.escape="cancel()">
